...
{    
    ...
    for(i=0; i<root_elec->m;i++)
    {
        ...
        {   
         ...
         /* //! Activate only for the backward wave
        if(x>=(x01-c*t))
        { 
           
            sine2 = sin(OMEG*(t+(x-x01)*inv_c)); //! backward wave
        }
        if(x>=(x01-c*(t+dt))) 
        {   
           
            sine3 = sin(OMEG*(t+dt+(x-x01)*inv_c)); //backward wave
        }
        */
        ...
        {
            //!-------- for forward + backward wave -------------
            //eyi[i][j] =   E0*(sine+sine2);   //! incident field (forward direction+backward direction) at t
            //eyi1[i][j] =  E0*(sine1+sine3);  //! incident field (forward direction+backward direction) at t+dt
            //!-----------------------------
	...
	for(i=0; i<child_elec->m;i++)
    {
        ...
        sine1=0.0;
            ...
            { 
                ...
                sine2 = sin(OMEG*(newt+(x-x01)*inv_c)); //!backward wave         
                
            }         
            if(x>=(x01-c*(newt+c_dt))) 
            {   
              
               
                sine3 = sin(OMEG*(newt+c_dt+(x-x01)*inv_c)); //backward wave
            }
           */

          for(j=0;j<child_elec->n;j++)
    	    {
      	      //  c_eyi[i][j] =   E0*(sine+sine2);  //! incident field (forward + backward direction) at newt
            //  c_eyi1[i][j] =  E0*(sine1+sine3); //! incident field (forward + backward direction) at newt +c_dt
        ...
        for(j=0;j<child_elec->n;j++)
        {
            ...
            {
                extk=c_ext[i][j];
                //!Scattered E-field (child) x update depends on previous E-data, previous H-data (top/bottom), density data (space:left/ right),velocity (same location)
            ...
            if(i>0)
            {
                ...
                //!Scattered E-field (child) y update depends on previous E-data, previous H-data (left/right), density data (space:top/ bottom),velocity (same location)
